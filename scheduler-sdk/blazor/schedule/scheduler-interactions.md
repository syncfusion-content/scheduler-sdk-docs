...
The following table describes the Blazor Scheduler actions and shows how they are performed with mouse and touch interactions.
...
| Date navigation | Click the previous or next navigation icons in the header bar to move between dates. | Swipe the Blazor Scheduler viewport left or right to navigate between dates on touch devices. Swiping does not work when a horizontal scroller is present in Scheduler. You can also use the previous and next navigation icons in the header bar. |
| View navigation | Click an event and drag it over the Blazor Scheduler to perform drag and drop. | The view options are available in the popup at the top-right corner of the header bar. Choose the required view from there. |

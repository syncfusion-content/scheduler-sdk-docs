---
layout: post
title: Accessibility in ASP.NET MVC Scheduler | Syncfusion
...
# Accessibility in ASP.NET MVC Scheduler
...
| aria-label | Attribute is set to the Scheduler parent element and its default value is the Scheduler's current date. Whenever the date is navigated, this attribute is updated with the appropriate current date value. It is also assigned to other Scheduler UI elements such as the previous and next date navigation buttons (depicting their purpose), the div element displaying the date range in the header bar, and appointment elements. |
| aria-labelledby | It indicates the editor dialog title to the user through assistive technologies. |
| aria-describedby | It indicates the editor dialog content description to the user through assistive technologies. |
...
| <kbd>Shift</kbd> + <kbd>Tab</kbd> | Reverse focusing of the `Tab` key functionality. Inverse focusing of event elements from the last one, then moves onto the first or active item on the Scheduler header bar, and then moves out of the component. |
...
| <kbd>Arrow</kbd> | To move onto the next available cells in any of the needed directions (left, right, top, and bottom). |
...
| <kbd>Alt</kbd> + <kbd>Number</kbd> (from 1 to 6) | To switch between the views of the Scheduler. |
...
The Scheduler component's accessibility levels are ensured through the [accessibility-checker](https://www.npmjs.com/package/accessibility-checker) and [axe-core](https://www.npmjs.com/package/axe-core) software tools during automated testing.
